...
// Module Name: fifo_xpm_async 
//////////////////////////////////////////////////////////////////////////////////
//fifo_xpm_async
//   #(.WRITE_DATA_WIDTH   (8       ), //W/R ratio must be 1:1, 1:2, 1:4, 1:8, 8:1, 4:1 , 2:1  
//     .READ_DATA_WIDTH    (8       ), //W/R ratio must be 1:1, 1:2, 1:4, 1:8, 8:1, 4:1 , 2:1  
...
//
//   fifo_xpm_async_u(
//   .WR_RST_I       (wr_rst         ),//at least 1 WR_CLK_I period
//   .WR_CLK_I       (wr_clk         ),//[0:0]
//   .WR_EN_I        (wr_en          ),//[0:0]
//   .WR_DATA_I      (wr_data        ),//[WRITE_DATA_WIDTH-1:0]
//   .WR_FULL_O      (wr_full        ),//[0:0]
//   .WR_DATA_COUNT_O(wr_data_count  ),//[WR_DATA_COUNT_WIDTH-1:0]
//   .WR_RST_BUSY_O  (wr_rst_busy    ),
//   .RD_CLK_I       (rd_clk         ),//[0:0]
//   .RD_EN_I        (rd_en          ),//[0:0]
//   .RD_DATA_O      (rd_data        ),//[READ_DATA_WIDTH-1:0]
//   .RD_EMPTY_O     (rd_empty       ),//[0:0]
//   .RD_DATA_COUNT_O(rd_data_count  ),//[RD_DATA_COUNT_WIDTH-1:0]
//   .RD_RST_BUSY_O  (rd_rst_busy    ),
//   );

    
module fifo_xpm_async(
...
output WR_ALMOST_FULL_O,
output WR_PROG_FULL_O,
...
output RD_ALMOST_EMPTY_O,
output RD_PROG_EMPTY_O,
...
//must>=16 ; actual depth = WRITE_FIFO_DEPTH - 1;  must be power of two
parameter WR_DATA_COUNT_WIDTH = 10;
parameter RD_DATA_COUNT_WIDTH = 10;
parameter PROG_EMPTY_THRESH   = 0;  
parameter PROG_FULL_THRESH    = WRITE_FIFO_DEPTH;


localparam INNER_WR_DATA_COUNT_WIDTH =   $clog2(WRITE_FIFO_DEPTH) + 1;
localparam INNER_RD_DATA_COUNT_WIDTH =  (WRITE_DATA_WIDTH>=READ_DATA_WIDTH) ?
                                          ( $clog2(WRITE_FIFO_DEPTH*(WRITE_DATA_WIDTH/READ_DATA_WIDTH)) + 1 ) :
                                          ( $clog2(WRITE_FIFO_DEPTH/(WRITE_DATA_WIDTH/READ_DATA_WIDTH)) + 1 ) ;
...
assign RD_DATA_COUNT_O = {0,rd_data_count};
  ...
  xpm_fifo_async #(
      ...
      .FIFO_WRITE_DEPTH(WRITE_FIFO_DEPTH), //16~ //|Defines the FIFO Write Depth, must be power of two.      
      ...
      .READ_MODE("std"),         // String 默认标准模式  “std” “fwft”
      .RELATED_CLOCKS(0),        // DECIMAL  wr时钟和 rd时钟 是否同源（可以不同频）
   ...
   xpm_fifo_async_inst (
      ...
      .empty(RD_EMPTY_O),                 // 1-bit output: Empty Flag: When asserted, this signal indicates that the
                                     // FIFO is empty. Read requests are ignored when the FIFO is empty,
                                     // initiating a read while empty is not destructive to the FIFO.
      .full(WR_FULL_O),                   // 1-bit output: Full Flag: When asserted, this signal indicates that the
      ...
      .overflow( ),           // 1-bit output: Overflow: This signal indicates that a write request
                                     ...
                                     // FIFO.
      .prog_empty(RD_PROG_EMPTY_O),       // 1-bit output: Programmable Empty: This signal is asserted when the
                                     // number of words in the FIFO is less than or equal to the programmable
                                     // empty threshold value. It is de-asserted when the number of words in
                                     // the FIFO exceeds the programmable empty threshold value.
      .prog_full(WR_PROG_FULL_O),         // 1-bit output: Programmable Full: This signal is asserted when the
